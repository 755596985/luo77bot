...
class PluginManager
{
    ...
    /** @var array<string,bool> 已完成依赖注入（register 调用）的插件 */
    ...
    public function enable(string $name): bool
    {
        ...
        // 修复：插件若在禁用状态下加载，register() 未注入依赖（如 $logger），
        // 直接调用 enable() 会因 typed property 未初始化而报错。
        // 此处首次补一次 register()，完成依赖注入与事件注册。
        if (empty($this->initialized[$name])) {
            $plugin->register($this->dispatcher, $this->logger);
            $this->initialized[$name] = true;
        }
