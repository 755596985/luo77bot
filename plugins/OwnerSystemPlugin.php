...
MD;
    ...
    private function executeCommand(C2CMessageEvent|GroupAtMessageEvent $event, string $content): void
    {
        ...
        // 安全限制（已强化）：
        // 1) 拒绝任何 shell 元字符，杜绝命令注入 / 串联执行；
        // 2) 仅允许白名单内的基础命令；
        // 3) 保留关键字黑名单作为兜底。
        if (preg_match('/[;&|`$()<>{}\n\r\t\\\\]/', $command)) {
            $event->replyText('🚫 出于安全考虑，已禁止包含 shell 元字符（; & | ` $ ( ) < > 等）的命令。');
            $this->logger->warning("主人尝试执行含元字符的命令: {$command}");
            return;
        }
        $baseCmd = strtok(trim($command), ' ');
        $allowed = ['php', 'git', 'composer', 'ls', 'cat', 'tail', 'head', 'pwd', 'date', 'df', 'free', 'ps', 'uptime', 'whoami', 'uname', 'crontab', 'du'];
        if (!in_array($baseCmd, $allowed, true)) {
            $event->replyText('🚫 该命令不在允许执行白名单内（仅允许: ' . implode('/', $allowed) . '）。');
            $this->logger->warning("主人尝试执行非白名单命令: {$command}");
            return;
        }
        $dangerous = ['rm -rf', 'dd', 'mkfs', 'format', 'del /f', 'rd /s', 'shutdown', 'reboot'];
