...
管理后台已全面重构美化，部署入口：`admin.php`，集成了以下核心功能：

### 人设系统
- 多人设管理：为机器人配置多套人设，一键套用预设人设，也可完全自定义
- 人设改名：随时修改机器人的名字（name 字段），无需改动代码
- 记忆系统：人设自带记忆能力，机器人可基于上下文连续对话
- 情感识别：自动识别用户消息中的情感倾向，人设据此调整回复语气

### AI 对接
- 可视化配置：后台直接配置 AI 服务商，无需手改代码
- 多供应商 + 故障转移：可同时配置多个 AI 供应商（商汤 / DeepSeek / OpenAI / 通义千问 等，均兼容 OpenAI Chat Completions 格式）。支持「故障转移」与「加权轮询」两种策略——某个供应商被限速(429)或宕机时，自动切换到下一个，避免「只有一个、限速就挂」
- 策略说明：故障转移（按列表顺序，前一个失败切下一个）；加权轮询（按权重随机分散请求，失败再切换）
- 后台「测试全部」：一键检测所有已配置供应商的连通性
- 支持服务商：商汤、DeepSeek、OpenAI、通义千问 等主流大模型
- 多模型切换：可同时配置多家服务，按需切换或负载分配

### 命令中心
- 自动扫描所有插件的中文命令关键词，集中展示
- 统一管理机器人可响应的全部指令，方便查看与维护

### 功能管理
- 模块开关：后台可视化开关各功能模块，即开即用
- 插件文件管理：支持在后台可视化删除插件文件，无需 SSH

### 回调推送
- 第三方推送：支持将订单模板等业务数据，通过回调推送至机器人私聊
- 配置入口：`callback.php`，可视化配置推送规则与模板
